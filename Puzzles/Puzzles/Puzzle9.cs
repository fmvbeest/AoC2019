...
public class Puzzle9 : PuzzleBase<IEnumerable<long>, long, long>
{
    protected override string Filename => "Input/puzzle-input-09.txt";
    protected override string PuzzleTitle => "--- Day 9: Sensor Boost ---";
    
    public override long PartOne(IEnumerable<long> input)
    {
        var virtualMachine = new IntcodeComputer(new IntcodeConfig { InputValue = 1, MemSizeFactor = 10}, 
            input.ToArray());
    ...
    {
        var virtualMachine = new IntcodeComputer(new IntcodeConfig { InputValue = 2, MemSizeFactor = 10}, 
            input.ToArray());
